...
public class AmexCreditCardGenerator implements CreditCardGenerator {
	...
	@Autowired
	public AmexCreditCardGenerator(CCSuffix ccSuffix) {
		super();
	...
	public String generateCCNumber() {
		return ccSuffix.getAmexSuffix() + ThreadLocalRandom.current().nextInt(100000000, 999999999 + 1);
	}
	...
	public List<CreditCardDetailsImpl> filterAndGenerateValidCreditCards(final List<String> ccNumbers)
		...
		for (String ccNumber : ccNumbers) {
			executor.submit(() -> {
				try {
					lock.lock();
					if (ccNumber.startsWith(ccSuffix.getAmexSuffix())) {
						creditCardList.add(new CreditCardDetailsImpl(ccNumber, CardTypes.AMEX.getValue(), new Date()));
					}
				} finally {
					lock.unlock();
				}
